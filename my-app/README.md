...
- public
  ...
  - index.html <!-- Our entire react code gets injected here div id="root"></div> -->
    - title
    - fonts
    - css
    - favicon
...
root.render(<Greeting />) //in react you always self close the component tag
```
...
```js
const Greeting = () => {
  // more difficult for complex component
...
const Greeting = () => {
  // We have to do all this to do what we did above.
...
- return single element (one parent element)
  ...
  - semantics section/article. You can use div but its recommeneded to use HTML semantics elements.
  - Fragment - let's us group elements without adding extra nodes. Fragments:
...
- className instead of class. Eventually in browser its setup as class.

```js
return <div className="someValue">hello</div>
```

- close every element. All elements that dont close we need to self close them.
...
```js
import './index.css' //Must provide file extension
...
- value inside must be an expression (return value),
  can't be a statement like {let x = 6}
...
const Book = () => {
  ...
  return (
    <article className="book">
      ...
      {/* <p>{let x = 6}</p> */} //This wont work and show us an error as its a statement
      <p>{6 + 6}</p>
    </article>
  )
}
